...
interface SocketState {
  ...
  cleanup: () => void;
  getRole: () => "user" | "tutor";
  createChat: (participants: { participantId: string; role: "user" | "tutor" }[]) => Promise<string>;
...
const socketStore = create<SocketState>()(
    ...
    (set, get) => ({
        ...
        try {
          ...
          return new Promise<void>((resolve, reject) => {
            ...
            socket.on("receiveMessage", (message: Message) => {
              ...
              if (currentChatId === message.chatId && !messages.some(msg => msg.id === message.id)) {
      ...
      createChat: async (participants: { participantId: string; role: "user" | "tutor" }[]) => {
        try {
          const targetParticipant = participants.find(p => p.participantId !== get().senderId);
          if (!targetParticipant) throw new Error("Invalid participants");
          const existingChat = get().findExistingChat(targetParticipant.participantId);
          if (existingChat) {
            await get().initializeChat(existingChat.id);
            return existingChat.id;
          }
          const role = get().getRole();
          const { chatId, recipient, senderId, senderRole } = await createChat(participants, role);
          set(state => ({
...
joinChat: (chatId: string) => {
  ...
  initializeChat: async (chatId: string) => {
    try {
      ...
      const participants = chatDetails.map(detail => detail.participantId || detail);
      ...
      if (recipient) {
        set({
          ...
          recipientProfilePicture: recipient.profilePhoto || 'defaultProfilePic.jpg',
        });
      }
  ...
  updateChatList: (message: Message) => {
    set((state) => {
      ...
      const chatExists = state.chatList.some(chat => chat.id === message.chatId);
      if (!chatExists) {
        
        return {
          chatList: [...state.chatList, { id: message.chatId, participants: [], lastMessage: message, unreadCount: 1 }]
        };
      }
      return{
      chatList: state.chatList.map(chat => {
