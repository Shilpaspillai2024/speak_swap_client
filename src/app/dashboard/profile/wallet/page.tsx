...
const WalletPage: React.FC = () => {
  ...
  useEffect(() => {
    const getWalletDetails = async () => {
      try {
        const walletData = await fetchUserWallet(currentPage, itemsPerPage);
        console.log("walletData", walletData);
        setBalance(walletData.data.wallet.balance || 0);
        setTransactions(walletData.data.wallet.transactions || []);
        setTotalItems(walletData.data.meta.totalItems);
        setTotalPages(walletData.data.meta.totalPages);
        setCurrentPage(walletData.data.meta.currentPage);
      } catch (error) {
              ...
              <>
                <div className="space-y-4">
                  {transactions.map((txn, index) => (
                    <div
                      key={index}
                      className="flex items-center p-4 border rounded-lg hover:shadow-md transition-shadow duration-200 bg-gray-50"
                    >
                      <div className="mr-4">{getTransactionIcon(txn.type)}</div>
